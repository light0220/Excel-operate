...
class ExcelOperate:
    ...
    def insert_rows(self, idx: int, amount: int = 1, height: float = 'before'):  # 定义插入行模块
        ...
        if len(merged_cells) > 0:  # 判断是否存在合并单元格
            ...
            for merged_cell in merged_cells:
                ...
                self.ws.unmerge_cells(start_row=merged_cell.min_row, end_row=merged_cell.max_row,
                                      start_column=merged_cell.min_col, end_column=merged_cell.max_col)  # 解除合并单元格

            self.ws.insert_rows(idx, amount)  # 插入空白行

            for i in merged_cell_list:
                self.ws.merge_cells(start_row=i['min_row'], end_row=i['max_row'],
                                    start_column=i['min_col'], end_column=i['max_col'])  # 重新合并单元格

        else:
            self.ws.insert_rows(idx, amount)  # 插入空白行

        # 从插入行的上方复制单元格格式
        for src_row in self.ws.iter_rows(min_row=idx-1, max_row=idx-1):
            for tag_row in self.ws.iter_rows(min_row=idx, max_row=idx+amount-1):
                for src_cell, tag_cell in zip(src_row, tag_row):
                    ...
                    tag_cell.number_format = copy(
                        src_cell.number_format)  # 复制数据格式
    ...
    def insert_cols(self, idx: int, amount: int = 1, width: float = 'before'):  # 定义插入列模块
        ...
        for i in range(idx, self.ws.max_column+1):
            col_width_list.append(
                self.ws.column_dimensions[get_column_letter(i)].width)
        ...
        # 从插入列的前方复制单元格格式
        for src_col in self.ws.iter_cols(min_col=idx-1, max_col=idx-1):
        ...
        for i in range(idx, idx+amount+1):  # 设置插入列的列宽
            if width == 'before':
                self.ws.column_dimensions[get_column_letter(
                    i)].width = self.ws.column_dimensions[get_column_letter(idx-1)].width
            else:
